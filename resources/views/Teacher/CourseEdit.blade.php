        ...
        <div>
            ...
            <div style="display:flex; flex-direction:column; align-items:center;">
                ...
                <textarea name="course_description" id="course_description" autocomplete="off"
                    style="height:150px; width:80%; font-size:16px; padding:10px; resize:vertical;" required="">{{ $course->description }}</textarea>
            </div>
            <br>
            <!-- Price Section -->
            <div style="display:flex; flex-direction:column; align-items:center; margin-bottom:10%;">
                ...
                <div style="position:relative; width: fit-content; height:fit-content;">
                    <input type="number" name="course_price" id="course_price" value="{{ $course->price ?? 0 }}"
                        ...
                        style="height:20%; text-align:center; font-size:40%; width:fit-content; padding-left:20px;"
                        required>
                    <span
                        style="position: absolute; left: 8px; top: 60%; transform: translateY(-50%); font-size:40%; color: black; pointer-events: none;">$</span>
                </div>
            </div>
            @error('course_price')
            ...
            <div
                ...
                <div>
                    ...
                    <span style="margin-left: 10px;">
                        {{ $course->is_paid ?? false }} ? {{ __('messages.purchasableWithSparkies') }} :
                        {{ __('messages.unpurchasableWithSparkies') }}
                    </span>
                </div>
                <label class="switch">
                    <input type="checkbox" name="course_paid" id="course_paid" value="1"
                        {{ $course->sparkies ?? false ? 'checked' : '' }}>
