...
<nav class="NavBar">
    ...
    <div class="mobile-menu" id="mobileMenu">
        ...
        <form action="/logout" method="POST" class="mobile-logout-form" style="margin-bottom:1rem;">
            @csrf
        ...
        <div class="button-container">
            ...
            @if (Auth::user()->privileges == 2)
                <a href="/admin/course-requests/show" style="position:relative;width:1rem;" target="_blank">
                    <span class="material-symbols-outlined">inbox</span>
                    @if ($pendingRequests > 0)
                        <span class="nav-count"
                            style="position:absolute;top:0;right:0;transform:translate(40%,0%);background:#e74c3c;width:2rem;height:1.6rem;">{{ $pendingRequests }}</span>
                    @endif
                </a>
                @php $pendingReports = App\Models\Report::where('status', 'PENDING')->count(); @endphp
                <a href="/reports" style="position:relative;width:1rem;" target="_blank">
                    <span class="material-symbols-outlined" style="color:white;">report</span>
                    @if ($pendingReports > 0)
                        <span class="nav-count"
                            style="position:absolute;top:0;right:0;transform:translate(40%,0%);background:#e74c3c;width:2rem;height:1.6rem;">{{ $pendingReports }}</span>
                    @endif
                </a>
            @endif
    ...
    <div class="NavBarElement" style="margin-right: 5%;">
        ...
        @elseif (Auth::user()->privileges == 0)
            ...
            <a href="/courses" class="NavBarText" id="Link">
                {{ __('messages.yourCourses') }}
                <span class="nav-count">{{ $teacher->courses->count() }}</span>
            </a>
            <a href="/lectures" class="NavBarText" id="Link">
                {{ __('messages.yourLectures') }}
                <span class="nav-count">{{ $lecCount }}</span>
            </a>
        @endif
