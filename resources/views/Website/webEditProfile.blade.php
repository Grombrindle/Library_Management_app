                ...
                <form method="POST" action="{{ route('web.profile.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-col items-center mb-8">
                        <div class="relative mb-4">
                            <div class="w-32 h-32 rounded-full bg-cover bg-center border-4 border-[#b0b0cf]"
                                 style="background-image: url('{{ App\Models\User::findOrFail(session('user'))->avatar ? asset('storage/'.App\Models\User::findOrFail(session('user'))->avatar) : 'https://via.placeholder.com/150' }}')"></div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div>
                            ...
                            <input type="text" name="userName" value="{{ old('userName', App\Models\User::findOrFail(session('user'))->userName) }}"
                            ...
                            <div class="flex gap-2">
                                <select name="countryCode" class="w-1/3 px-4 py-3 border border-gray-300 rounded-lg">
                                    @foreach(['+1', '+20', '+33', '+49', '+90'] as $code)
                                        <option value="{{ $code }}" {{ $code == App\Models\User::findOrFail(session('user'))->countryCode ? 'selected' : '' }}>{{ $code }}</option>
                                    @endforeach
                                </select>
                                <input type="text" name="number" value="{{ old('number', App\Models\User::findOrFail(session('user'))->number) }}"
