            ...
            <section class="relative bg-gradient-to-br from-[#b0b0cf] to-[#d8d8e7] text-white py-24">
                <div class="container mx-auto px-6">
                    <div class="max-w-3xl text-center md:text-left">
                        ...
                        <p class="text-xl mb-8 opacity-90 text-[#202025]">Find the perfect course to expand your knowledge and skills, taught by industry experts.</p>
                    </div>
                </div>
            </section>
    ...
    <div class="container mx-auto px-6">
        ...
        @php
            ...
            $featuredCourseData = [
                ...
                'image' => $featuredCourse->image ? asset($featuredCourse->image) : 'https://via.placeholder.com/800x600',
                'teacher' => [
                    'id' => $featuredCourse->teacher->id,
                    'name' => $featuredCourse->teacher->name,
                    'image' => $featuredCourse->teacher->image ? asset($featuredCourse->teacher->image) : 'https://via.placeholder.com/150',
                ],
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden lg:flex cursor-pointer"
            ...
            <div class="p-8 lg:w-1/2 flex flex-col justify-center">
                    ...
                    <div class="text-right">
                         @if($ratingValue > 0)
                            <div class="flex items-center">
                                ...
                                <span class="text-2xl font-bold text-[#b0b0cf] mr-2">{{ number_format($ratingValue, 1) }}</span>
                                @for ($i = 1; $i <= 5; $i++)
                                    {{-- Use the accurate rounded rating value for stars --}}
                                    <i class="fas fa-star {{ $i <= round($ratingValue) ? 'text-yellow-400' : 'text-gray-300' }}"></i>
                                @endfor
                            </div>
                ...
                <div class="bg-[#b0b0cf] text-white px-8 py-4 rounded-lg font-semibold hover:bg-[#8a8aac] transition-colors w-full text-center">
                    View Course Details
                </div>
            </div>
        </div>
...
@endif
        ...
        <div 
            ...
            x-on:open-course-modal.window="openModal($event.detail.course)"
                ...
                <template x-if="currentCourse">
                    <div class="overflow-y-auto">
                        <div class="flex items-start justify-between p-5 border-b border-gray-200 rounded-t-xl">
                            <div class="flex-grow">
                                ...
                                <p class="text-sm text-gray-500 mt-1">
                                    By <span x-text="currentCourse.teacher.name"></span> in <span class="font-semibold" x-text="currentCourse.subject ? currentCourse.subject.name : 'General'"></span>
                                </p>
                            </div>
                            <button @click="closeModal()" class="p-2 text-gray-400 rounded-full hover:bg-gray-100 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#b0b0cf]">
                        ...
                        <div class="p-6 space-y-6">
                            <img :src="currentCourse.image" :alt="currentCourse.name" class="w-full h-56 object-cover rounded-lg">
