<x-layout>
    <x-addcard link="addexam" object="Exam">
        <div style="display:flex; flex-direction:column; align-items:center;">
            <label for="exam_title">
                {{ __('messages.examTitle') }}:
            </label>
            <input type="text" name="title" id="exam_title" value="{{ old('title') }}" autocomplete="off"
        ...
        <div style="display:flex; flex-direction:column; align-items:center; height:100%;">
            ...
            <div style="position: relative; width: 80%;">
                <textarea name="description" id="exam_description" autocomplete="off"
                    ...
                    maxlength="500" oninput="updateCharCount(this, 500)" required>{{ old('description') }}</textarea>
                <div id="charCount"
        ...
        <div style="display:flex; flex-direction:column; align-items:center;">
            ...
            <input type="date" name="date" id="exam_date" value="{{ old('date') }}"
                style="height:20%; text-align:center; font-size:40%; width:fit-content;" required>
        </div>
        @error('date')
            <div class="error">{{ $message }}</div>
        ...
        <div class="file-upload-section">
            ...
            <div style="display:flex; flex-direction:column; align-items:center;">
                <label for="pdf_file">
                    {{ __('messages.examPdfFile') }}:
                </label>
                <div class="custom-file-input">
                    <input type="file" id="pdf_file" class="hidden-file-input" name="pdf_file"
                        accept=".pdf" required>
                    <label for="pdf_file" class="file-input-label">
                        <span class="file-input-text" id="pdf-text">{{ __('messages.chooseFile') }}</span>
                    </label>
                </div>
                <small style="color: #666; margin-top: 5px;">{{ __('messages.pdfSizeWarning') }}</small>
            </div>
            @error('pdf_file')
                <div class="error">{{ $message }}</div>
            @enderror
