        ...
        <div>
            <div style="display:flex; flex-direction:column; align-items:center;">
                ...
                <input type="text" name="admin_name" id="admin_name" value="{{ old('admin_name') ?? $admin->name }}"
                    style="height:20%; text-align:center; font-size:40%; width:fit-content; margin-bottom:10%; @error('admin_name') border:2px solid red @enderror;">
            </div>
            @error('admin_name')
            ...
            <div style="display:flex; flex-direction:column; align-items:center;">
                ...
                <input type="text" name="admin_user_name" id="admin_user_name"
                    value="{{ old('admin_user_name') ?? $admin->userName }}"
                    style="height:20%; text-align:center; font-size:40%; width:fit-content; margin-bottom:10%; @error('admin_user_name') border:2px solid red @enderror;">
            </div>
            ...
            <div style="display:flex; flex-direction:column; align-items:center;">
                ...
                <div
                    ...
                    <input type="text" name="admin_number" id="admin_number" placeholder="9XXXXXXXX"
                        value="{{ old('admin_number') ?? $admin->number }}" autocomplete="off" inputmode="numeric"
            ...
            <div class="dropdown-container">
                ...
                <select id="admin-dropdown" name="admin_privileges" class="dropdown"
                    style="padding:1rem 2.4rem; text-align:left">
                    @if (old('admin_privileges') === 'Administrator')
                        <option selected>{{ __('messages.admin') }}</option>
                        <option>{{ __('messages.semi-admin') }}</option>
                        1st
                    @elseif (old('admin_privileges') === 'Semi-Administrator')
