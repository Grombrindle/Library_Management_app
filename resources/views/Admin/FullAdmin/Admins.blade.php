...
<x-layout :objects=true object="{{__('messages.admins')}}">
    <x-breadcrumb :links="[__('messages.home') => url('/welcome'), __('messages.admins') => url('/admins')]" />
                ...
                <div class="chunk">
                    @foreach ($chunk as $admin)
                        <x-card link="admin/{{ $admin->id }}" image="{{ asset($admin->image) }}" object="Admin">
                            ...
                            ● {{__('messages.adminNumber')}}: <span style="direction: ltr; display: inline-block;">&emsp;{{ $admin->countryCode }} {{ $admin->number }}</span>
